...
def test_format_date_by_name():
    ...
    assert l10n.format_date(d, "short", locale="en") == "4/1/07"
    assert l10n.format_date(d, "medium", locale="en") == "Apr 1, 2007"
    assert l10n.format_date(d, "long", locale="en") == "April 1, 2007"
    assert l10n.format_date(d, "full", locale="en") == "Sunday, April 1, 2007"

    assert l10n.format_date(d, "...", locale="en") == "..."
    assert l10n.format_date(d, locale="en") == "Apr 1, 2007"
...
def test_set_date_formats():
    ...
    d = date(2007, 4, 1)
    assert l10n.format_date(d, locale="en") == "4/1/07"

    l10n.set_date_formats({"date": "full", "date.short": "'trolololo'"})
    assert l10n.format_date(d, locale="en") == "Sunday, April 1, 2007"
    assert l10n.format_date(d, "short", locale="en") == "trolololo"
...
def test_format_timedelta_direction_forward():
    ...
    expected = "dentro de 1 semana"
    assert l10n.format_timedelta(delta, locale="es_PE", add_direction=True) == expected

    expected = "dentro de 1 mes"
    assert (
        l10n.format_timedelta(
            delta, granularity="month", locale="es_PE", add_direction=True
        ).lower()
        == expected
    )
...
def test_format_decimal():
    ...
    assert l10n.format_decimal(1.2345, locale="en_US") == "1.234"
    assert l10n.format_decimal(1.2345, locale="sv_SE") == "1,234"
    assert l10n.format_decimal(12345, locale="de_DE") == "12.345"
    assert l10n.format_decimal(-1.2345, format="#,##0.##;-#", locale="en") == "-1.23"
...
def test_format_currency():
    ...
    assert l10n.format_currency(1099.98, "USD", locale="en_US") == "$1,099.98"
    assert l10n.format_currency(1099.98, "CAD", locale="en_US") == "CA$1,099.98"
    assert l10n.format_currency(1099.98, "EUR", locale="de_DE") == "1.099,98\xa0\u20ac"
    assert l10n.format_currency(1099.98, "EUR", locale="en_US") == "\u20ac1,099.98"
    assert (
        l10n.format_currency(1099.98, "EUR", "\xa4\xa4 #,##0.00", locale="en_US")
        == "EUR 1,099.98"
    )
...
def test_format_percent():
    ...
    assert l10n.format_percent(0.34, locale="en_US") == "34%"
    assert l10n.format_percent(25.1234, locale="en_US") == "2,512%"
    assert l10n.format_percent(25.1234, locale="sv_SE") == "2\xa0512\xa0%"
    assert l10n.format_percent(25.1234, "#,##0\u2030", locale="en_US") == "25,123\u2030"
